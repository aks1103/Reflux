   ...
   if ($retval->num_rows == 1)//checks only one registration with an email id
   {
    while($row = $retval->fetch_assoc()) {
      ...
      if ($db->query($sql) === TRUE) {
        ...
        header("Location:error.php?error_id=S104");
      ...
      $mail->Body    = "Dear participant,Your E-mail has been successfully confirmed! <a href='http://enigma.reflux.in/order.php?id=100&mail=".htmlentities($email)."&phone=".htmlentities($phone)."&name=".htmlentities($name)."'>Click on the provided linkn to proceed for the payment procedure</a>Once payment is confirmed login to your email using the following  password:".$pass[0];
    ...
    //   echo "<html>Confirmation Mail is send to your account. Confirm your account.";
       ...
       exit();
        ...
        header("Location:error.php?error_id=S101");
...
if($state == 2){
        header("Location:error.php?error_id=S102");
...
<div class="form-group has-feedback">
<input name="phone" type="text" class="form-control" placeholder="Contact No." pattern="[1-9][0-9]{9}" title="Enter a valid phone number">
...
<label>
<input type="checkbox" required title="Please accept terms and condition to continue">I agree to the <a href="terms.pdf">terms</a>
</label>
</div>
</div>
<!-- /.col -->
